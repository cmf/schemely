 ...
 * @author ilyas
 */
public class ClSymbolImpl extends SchemePsiElementImpl implements ClSymbol
{
  ...
  public String getReferenceName()
  {
    ...
    {
      // TODO CMF
      //      if (nameElement.getNode().getElementType() == Tokens.symATOM)
      //      {
  ...
  {
    final ASTNode newNode = SchemePsiElementFactory.getInstance(getProject()).createSymbolNodeFromText(newName);
  ...
  public static class MyResolver implements ResolveCache.PolyVariantResolver<ClSymbol>
  {
    public ResolveResult[] resolve(ClSymbol symbol, boolean incompleteCode)
    {
      ...
      ResolveProcessor
        processor =
        new SymbolResolveProcessor(StringUtil.trimEnd(name, "."), symbol, incompleteCode, nameString.endsWith("."));
                                                             ...
                                                             final boolean forCompletion)
    {
      ...
      List<MethodSignature>
        goodSignatures =
        ContainerUtil.findAll(sig2Method.keySet(), new Condition<MethodSignature>()
        {
      ...
      final HashSet<SchemeResolveResult> results = new HashSet<SchemeResolveResult>();
      for (MethodSignature signature : goodSignatures)
      {
        ...
        {
          results.add(new SchemeResolveResultImpl(method, true));
        }
      }

      return results.toArray(new SchemeResolveResult[results.size()]);
    }
